...
public class Simulation
{
   ...
   public void loadPassengers(Station station, Car car)
   {
      ...
      for(int i=0;i<station.peopleWaitingList().size();i++) {
    	  p = station.peopleWaitingList().get(i);    
    	  if(p.getDestination()==carDest && car.hasRoom()) {
    		  car.add(p);
      ...
      //Now, anyone else going on the way ("ride passenger").
    	  //&&car.hasRoom()
    	  //&&p.getDestination()>station.getNumber()
    	  else if(p.getDestination()<carDest && car.hasRoom()) {
    		  car.add(p);
      //.....
